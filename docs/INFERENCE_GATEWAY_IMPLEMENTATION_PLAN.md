...
5. Environment: run the pass against the Naver Cloud testbed —
   gateway on testbed-gw, OAM on testbed-client, UI local dev server
...
- Integration: full stack — either local docker-compose or the remote testbed
  (`docs/internal/TESTBED.md`, Workflow B: gateway on testbed-gw + OAM/UI/
  Prometheus on testbed-client) — with traffic generator; scripted checks of query

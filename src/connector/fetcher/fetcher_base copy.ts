...
async function fetch_data(url: string, options?: RequestOptions): Promise<Response> {
	...
	try {
		...
		if (resp.status === 401) {
			...
			// Check if this is an authentication endpoint - allow graceful error handling
			const isAuthEndpoint = url.includes('/users') || url.includes('/login') || url.includes('/oauth');
			
			if (!isAuthEndpoint) {
				// For non-auth endpoints, maintain original redirect behavior
				const resp_json = await resp.json();
				const code = resp.status;
				const message = resp_json.message || resp.statusText;
				move_500(code, message);
			}
			// For auth endpoints, return the response to allow graceful error handling
		}

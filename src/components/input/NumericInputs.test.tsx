...
describe('UI-P6-2 SecurityRate numeric fields', () => {
	...
	it('over-max input is an error, not silently clamped', async () => {
		const user = userEvent.setup();
		const onChange = renderForm();
		const field = screen.getByLabelText(/UDP Bandwidth/);
		...
		await user.tab();
		// RED today: TextBox has no max wired here and validateForm catches it,
		// but the value the parent holds was never clamped-transparently either —
		// after the fix the raw text must survive with a range error.
		expect(lastCall(onChange).isValid).toBe(false);
